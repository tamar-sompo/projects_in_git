...
function Invoice(props) {
  ...
  useEffect(() => {
    ...
    $(".step1").click()
    "useeeeeeeeeeeeeeeeeeeeee", detailsInvoice)
  // dispatchgetbusiness()
  ...
  if (history.location.pathname === `/${userName}/invoice`) {
    "useeeeeeeeeeeeeeeeeeeeee1111111")
    setInvoiceShow({})
  }
}, [])
...
"productss", detailsInvoice.products)
let productss = [...detailsInvoice.products]
productss.splice(index, 1);
dispatch(actions.setProductionAfterDelete(productss))
  ...
  let productSelect3 = allproduct.find(x => x._id === productSelect3id)
  "productSelect3", productSelect3)
  //  dispatch(actions.setPDelete(productSelect3.name))
  //    "productSelect3",productSelect3.name)
  "productSelect3", p)
  "tammmm", p.length, invoice.products.length)
...
"productSelect2", productSelect)
let productss = [...invoice.products]
"productss1111", productss)
productss.splice(index, 1)
"productss", productss)
          ...
          <Row className='m-0'>
            <Col md={8} className='' onClick={() => focus_steps('Production', 1)}>
              ...
              :detailscontact.contact.name */}
                {"detailsss", detailsInvoice)}
                {"detailssscontact", detailscontact)}
                {"detailssscontact", detailsInvoice && !detailscontact)}
...
export const Item = (props) => {
  ...
  const vv2 = (e) => {
    ...
    "evaluename", p)
    "tamar11", invoice.products.length)
    dispatch(actions.deleteLastProductInvoice());
    "tamar", invoice.products.length)
    dispatch(actions.setProducts(e.value._id));
    "yyyyyy", invoice.products)
